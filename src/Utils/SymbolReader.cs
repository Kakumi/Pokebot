...
namespace Pokebot.Utils
{
    ...
    {
        public static IReadOnlyList<byte> ReadSymbol(ApiContainer api, long address, int offset = 0x0, int size = 0x0)
        {
            return api.Memory.ReadByteRange(address + offset, size);
        }

        public static IReadOnlyList<byte> ReadSymbol(ApiContainer api, Symbol symbol)
        {
            return ReadSymbol(api, symbol.Address, 0, symbol.Size);
        }

        public static IReadOnlyList<byte> ReadSymbol(ApiContainer api, Symbol symbol, int offset = 0x0, int size = 0x0)
        {
            return ReadSymbol(api, symbol.Address, offset, size);
        }

        public static IReadOnlyList<Symbol> LoadSymbols(byte[] resource)
        {
